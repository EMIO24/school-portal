...
def owns_student(request, student_id, profile=False):
    ...
    if request.user.role == 'student': return student.user_id == request.user.pk
...
def require_assignment(request, class_arm, term=None, subject=None):
    if request.user.role != 'teacher': return
    ...
    if not qs.exists(): raise PermissionDenied('This class/subject is not assigned to you.')

class SchoolModulePermission(BasePermission):
    def has_permission(self, request, view):
        user=request.user; tenant=getattr(request,'tenant',None)
        if not user or not user.is_authenticated or not user.is_active or tenant is None or user.school_id != tenant.pk: return False
        if user.must_change_password: return False
        role,module,name=user.role,view.__module__.split('.')[0],type(view).__name__; action=getattr(view,'action','')
        if role=='school_admin': return True
        if module in ('notifications','promotion'): return False
        if module=='timetable': return request.method in SAFE_METHODS
        if module=='cbt':
        ...
        if module=='gradebook':
            # Teachers may work only inside their assigned class/subject/term.
            # Object destruction remains admin-only.
            return role=='teacher' and action!='destroy'
        ...
        if module=='analytics': return request.method in SAFE_METHODS and name in ('StudentTrendsView','TranscriptPDFView') and owns_student(request,view.kwargs.get('pk'),profile=True)
        return False
...
class TenantRelationsMixin:
    def validate(self, attrs):
        ...
        attrs=super().validate(attrs); request=self.context.get('request')
        if request is None: raise serializers.ValidationError('School context is required.')
        for name,value in attrs.items():
            ...
            for obj in values:
                ...
                if school_id is not None and school_id!=request.tenant.pk: raise serializers.ValidationError({name:'Select a record belonging to this school.'})
        arm=attrs.get('class_arm',getattr(self.instance,'class_arm',None)); term=attrs.get('term',getattr(self.instance,'term',None)); subject=attrs.get('subject',getattr(self.instance,'subject',None)); session=attrs.get('session',getattr(self.instance,'session',None))
        if term and session and term.session_id!=session.pk: raise serializers.ValidationError({'term':'Term must belong to the selected session.'})
        if arm: require_assignment(request,arm.pk,getattr(term,'pk',None),getattr(subject,'pk',None))
        return attrs
