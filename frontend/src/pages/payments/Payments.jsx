...
export function PlatformPayments() {
  const {user}=useAuth(); const [data,setData]=useState(null), [schools,setSchools]=useState([]), [filters,setFilters]=useState({status:'',kind:'',school_id:''}), [error,setError]=useState(''), [notice,setNotice]=useState(''), [busy,setBusy]=useState(false);
  const load=async (nextFilters=filters) => {try {const params=Object.fromEntries(Object.entries(nextFilters).filter(([, value]) => value));const p=await api.get('/api/platform/payments/',{params});setData(p.data);setSchools(p.data.schools || []);}catch(e){setError(message(e));}};
  ...
  {data.accounts.map(a => <p key={a.school_id}>{schools.find(s => s.id===a.school_id)?.name || a.school_id}: {a.business_name} / {a.bank_name} / ending {a.account_last_four}</p>)}<h2>Recent payments</h2><form onSubmit={e => {e.preventDefault();load();}}><label>Status<select value={filters.status} onChange={e => setFilters({...filters,status:e.target.value})}><option value="">All statuses</option><option value="initializing">Initializing</option><option value="pending">Pending</option><option value="review">Review required</option><option value="success">Success</option><option value="failed">Failed</option></select></label><label>Type<select value={filters.kind} onChange={e => setFilters({...filters,kind:e.target.value})}><option value="">All payments</option><option value="fees">School fees</option><option value="subscription">Subscription</option></select></label><label>School<select value={filters.school_id} onChange={e => setFilters({...filters,school_id:e.target.value})}><option value="">All schools</option>{schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label><button disabled={busy}>Filter</button></form><OrderList orders={data.orders} onRetry={async reference => {try {const r = await api.post("/api/platform/payments/", {reference,action:"retry_checkout"}); setNotice("Existing checkout: " + r.data.authorization_url); await load();} catch(e) {setError(message(e));}}} onVerify={async reference => {try {const r = await api.post("/api/platform/payments/", {reference});setNotice("Payment status: " + r.data.status);await load();} catch(e) {setError(message(e));}}}/></>}</main>;
}
